...
(function(){
  function NET_ERROR(str, code){util._errorMap[code] = str;}
...
var OTRSocket = function(ip, port, server, socketId) {
  this.ip = ip;
  this.port = port;
  this._buffers = [];
  if (server) {
...
OTRSocket.prototype.send = function(method, obj, callback) { //callback not implemented yet; request/response matching needed
...
OTRSocket.prototype._receive = function(json) {
  this.emit(json.method, json.payload);
};

OTRSocket.prototype._memcpyWhole = function(dst, dstOffset, src) {
  var dstU8 = new Uint8Array(dst, dstOffset, src.byteLength);
  var srcU8 = new Uint8Array(src, 0, src.byteLength);
  dstU8.set(srcU8);
};

OTRSocket.prototype._finalBuff = function(buff) {
...
OTRSocket.prototype._stringToArrayBuffer = function(str, callback) {
  var bb = new Blob([str]);
...
OTRSocket.prototype._arrayBufferToString = function(buf, callback) {
